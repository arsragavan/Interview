...
public class PrintAllPaths {
    ...
    public static void printPath(int[][] matrix, int i, int j, int row, int col,String path) {
        if (i == row-1 && j == col-1)
    ...
    public static int countwithUnreachable(int row, int col, int [][] reachable) {
        ...
        if (row < 1 || col < 1 || reachable [row-1][col-1] == 0)
        	return 0;
        return (countwithUnreachable(row-1, col,reachable)+countwithUnreachable(row, col-1,reachable));
    }
