...
public class BST {
    ...
    public boolean isBST(BST root, int min, int max) {
        ...
        if (root.value > min && root.value < max && isBST(root.left, min, root.value) && isBST(root.right, root.value, max)) {
            return true;
        }
        return false;
    }
    ...
    public boolean isLeft(BST root, BST node) {
        ...
        return (root.left == node || isLeft(root.left, node) || isLeft(root.right, node));
    }
    ...
    public boolean isDeep(BST root, BST node, int height) {
        ...
        return ((height == 1 && (root.left == node || root.right == node))
                || (isDeep(root.left, node, height - 1))
                || (isDeep(root.right, node, height - 1)));
    }
    ...
    public BST buildPrePost(int[] preOrder, int[] postOrder, int postStart, int postEnd, int n) {
    ...
    public BST buildPostIn(int[] postOrder, int[] inOrder, int inStart, int inEnd) {
        if (inStart > inEnd) {
            return null;
        }
    ...
    public BST inorderSuccessor(BST node) {
        ...
        while (root != null && root.right == node) {
            node = root;
            root = root.parent;
        }
        return root;
